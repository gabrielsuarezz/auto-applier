...
Greenhouse Job ID : 4353710003
Application URL   : https://job-boards.greenhouse.io/sigmacomputing/jobs/4353710003
Generated         : 2026-05-27
Mode              : DRY-RUN (submit() is intercepted and logged, NOT executed)

Phase 1 – Security scan : CLEAN (no injection directives detected)
Phase 2 – Gate check    : PASS  (New Grad role; Software Engineering domain; relocation OK)

Profile mapping used
--------------------
...
Primary project   : Voxtant (1st of 80+ at PlutoHacks 2025, full-stack, end-to-end ownership)
Secondary project : Helios AI (1st Knight Hacks VIII, CNN + autonomous systems)
Tertiary project  : ViewGuard (real-time security pipeline, async streams)
Pantheon          : NOT used (no AI/Agent swarm emphasis in posting)
GPA in text box   : No (already on PDF)
Cover letter slot : resume.pdf uploaded (no cover letter generated)
Salary field      : 90000
US work auth      : Yes
Visa sponsorship  : No
"""
...
# ── Configuration ───────────────────────────────────────────────────────────────
APPLY_URL    = "https://job-boards.greenhouse.io/sigmacomputing/jobs/4353710003"
...
LOG_FILE     = JOB_DATA_DIR / "processed" / "sigmacomputing_4353710003_dryrun.log"
...
async def intercept_submit(request: Request) -> None:
    """Capture any POST to a submit/application endpoint in DRY-RUN mode."""
...
async def run_application():
    ...
    async with async_playwright() as pw:
        ...
        # Location — Greenhouse may render a combined text field or a city/state pair
        ...
        # ── 3. Resume upload (CRITICAL — use PDF only) ───────────────────────
        ...
        if not resume_uploaded:
            log.warning("  ⚠️  No resume file input found — may require manual upload")
        ...
        for sel in cl_selectors:
            try:
                ...
                if await locator.count() > 0:
                    ...
                    log.info("  ✅ Cover letter slot filled with resume.pdf")
                    break
            except Exception:
                pass
        ...
        # GPA — only fill if an explicit GPA-labelled field exists (per policy)
        ...
        if await gpa_locator.count() > 0:
            ...
            log.info("  ℹ️  GPA field not present — already included on resume PDF")

        # ── 6. Eligibility / compliance questions ────────────────────────────
        log.info("--- Section: Eligibility ---")

        # US work authorization — answer Yes
        auth_selectors = [
        ...
        # Security clearance / citizenship eligibility — answer Yes if asked
        clearance_selectors = [
        ...
        # ── 9. Capture DOM state before submit ────────────────────────────────
        ...
        if DRY_RUN:
            ...
            if await submit_btn.count() > 0:
                ...
                log.warning(
                    "   Submit button found: %r — INTERCEPTED, not clicked.", btn_text.strip()
                )
            else:
...
def update_applied_jobs(dry_run: bool) -> None:
    ...
    entry = {
        ...
        "script"           : "apply_sigmacomputing_4353710003.py",

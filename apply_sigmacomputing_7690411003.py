...
Greenhouse Job ID : 7690411003
Application URL   : https://job-boards.greenhouse.io/sigmacomputing/jobs/7690411003
...
JMG bullet        : Default — Java + PostgreSQL enterprise system
Primary project   : Voxtant (1st/80+ PlutoHacks 2025)
Secondary project : Helios AI (1st Knight Hacks VIII)
Tertiary project  : ViewGuard (real-time security pipeline)
Pantheon          : NOT used (no AI/Agent emphasis in posting)
...
# ── Configuration ──────────────────────────────────────────────────────────────
APPLY_URL    = "https://job-boards.greenhouse.io/sigmacomputing/jobs/7690411003"
...
LOG_FILE     = JOB_DATA_DIR / "processed" / "sigmacomputing_7690411003_dryrun.log"
...
async def run_application():
    ...
    async with async_playwright() as pw:
        ...
        # Location — Greenhouse may show a single text field or a city/state pair
        ...
        # GPA — only fill if explicitly labelled (per policy: do not force into short-answer)
        ...
        # Security clearances / citizenship — answer Yes if asked
        clearance_sel = [
            "select[name*='clearance' i]",
            "select[name*='citizenship' i]",
            "select[name*='eligible' i]",
        ]
        for sel in clearance_sel:
        ...
        # ── 9. Capture final DOM state before submit ─────────────────────────
...
def update_applied_jobs(dry_run: bool) -> None:
    ...
    entry = {
        ...
        "script"           : "apply_sigmacomputing_7690411003.py",
